...
class DNAFountain:
    ...
    def save(self,file_name = 'out.dna'):
        logging.info("The length of a single chain is %d", len(self.dna_dl[0][0]))
        with open(file_name, 'w') as f:
            # f.write('Fountain code\n')
            # f.write('CN: ' + str(self.num_chunks) +'\n')
            # f.write('CL: ' + str(self.chunk_size) + '\n')
            # f.write('RS: ' + str(self.rs) + '\n')
            for i in range(len(self.dna_dl)):
                f.writelines(">packet {}_{}\n".format(i+1, self.dna_dl[i][2]))
                f.writelines("{}\n".format(self.dna_dl[i][0]))
            f.close()
...
class Glass:
    def __init__(self, in_file_name, chunk_num, pad_num, header_size = 4, 
    ...
    #---------------------------------------------add one droplet and preform decoding------------------------------------------------#
...
#---------------------------------------------writing encoding result to files------------------------------------------------#
    def String(self):
        res = ''
    ...
    def removePadding(self):
        crp = []
        for index in range(len(self.chunks[-1])-self.pad_num):
            crp.append(self.chunks[-1][index])
        self.chunks[-1] = crp
        return crp
    ...
    #------------------------------------------------decoding---------------------------------------------#
    def decode(self,show_solve_num = False):
